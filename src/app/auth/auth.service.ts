...
export class AuthService {
  ...
  login(email: string, password: string) {
    return this.http
      .post<AuthResponseData>(
        'https://www.googleapis.com/identitytoolkit/v3/relyingparty/verifyPassword?key=AIzaSyDkKTdqZ8QhCI4u6n4N53ZPFfwuI-DwB7M',
        {
          email: email,
          password: password,
          returnSecureToken: true,
  ...
  // Log in user if user data is stored in local storage
  autoLogin() {
    const userData: {
      email: string;
      id: string;
